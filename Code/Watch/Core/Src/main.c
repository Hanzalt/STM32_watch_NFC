...
/*
	  ...
	  **/
	  ...
	  if (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_9)) {
		  set_LED(leds, 7, led_pattern);
		  HAL_TIM_PWM_Start_DMA(&htim2, TIM_CHANNEL_1, (uint32_t *)leds, (7 * 24) + 72);
	  }

	  /*
	  if (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0) && isPressed == false) {
		  if (x<=11) {
			  x++;
		  } else {
			  x=0;
